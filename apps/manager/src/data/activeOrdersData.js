export const generateOrders = (count = 100) => {
    ...
    return Array.from({ length: count }, (_, i) => {
        ...
        return {
            ...
            timeElapsed: `${Math.floor(Math.random() * 12) + 1}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')} PM`,
            eta: `${Math.floor(Math.random() * 12) + 1}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')} PM`,
            progress: `${Math.floor(Math.random() * 99) + 1}`,
            isLate: `${Math.floor(Math.random() * 10) > 5}`,
            area: areas[Math.floor(Math.random() * areas.length)],
        };
    });
};

export const activeOrdersData = generateOrders(200);